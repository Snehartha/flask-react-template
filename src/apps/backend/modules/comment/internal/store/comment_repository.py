...
class CommentRepository(ApplicationRepository):
    ...
    def on_init_collection(cls, collection: Collection) -> bool:
        ...
        collection.create_index(
            [("task_id", 1), ("active", 1)], 
            name="task_id_active_index", 
            partialFilterExpression={"active": True}
        )
        
        # Index for finding comments by account
        collection.create_index(
            [("account_id", 1), ("active", 1)], 
            name="account_id_active_index", 
            partialFilterExpression={"active": True}
        )
        
        # Compound index for efficient queries
        collection.create_index(
            [("task_id", 1), ("account_id", 1), ("created_at", -1)], 
            name="task_account_created_index"
        )
