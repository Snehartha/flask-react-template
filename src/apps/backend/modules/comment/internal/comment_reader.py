...
from modules.comment.types import GetCommentParams, GetCommentsByTaskParams, Comment
...
class CommentReader:
    ...
    @staticmethod
    def get_comments_by_task(*, params: GetCommentsByTaskParams) -> PaginationResult[Comment]:
        filter_query = {
    ...
    @staticmethod
    def comment_exists_for_task(*, task_id: str, account_id: str) -> bool:
        """Check if any comments exist for a task"""
        count = CommentRepository.collection().count_documents({
            "task_id": task_id,
