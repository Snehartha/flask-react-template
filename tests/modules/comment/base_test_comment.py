...
class BaseTestComment(unittest.TestCase):
    ...
    def create_multiple_test_comments(self, task_id: str, account_id: str, count: int) -> list[Comment]:
        ...
        for i in range(count):
            comment = self.create_test_comment(
                task_id=task_id, account_id=account_id, content=f"Test comment {i+1}"
            )
    ...
    def make_authenticated_comment_request(
        ...
        if comment_id:
            ...
            raise ValueError("Either task_id or comment_id must be provided")
    ...
    def make_unauthenticated_comment_request(self, method: str, account_id: str, task_id: str = None, comment_id: str = None, data: dict = None):
        if comment_id:
            url = self.get_comment_by_id_api_url(account_id, comment_id)
        elif task_id:
            url = self.get_comment_api_url(account_id, task_id)
