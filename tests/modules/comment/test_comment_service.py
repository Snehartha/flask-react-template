...
class TestCommentService(BaseTestComment):
    ...
    def test_get_comment_for_account(self) -> None:
        ...
        get_params = GetCommentParams(account_id=self.account.id, comment_id=created_comment.id)
    ...
    def test_get_comments_by_task_empty(self) -> None:
        ...
        result = CommentService.get_comments_by_task(params=get_params)
    ...
    def test_get_comments_by_task_with_data(self) -> None:
        comments_count = 5
    ...
    def test_delete_comment(self) -> None:
        created_comment = self.create_test_comment(task_id=self.task.id, account_id=self.account.id)
        delete_params = DeleteCommentParams(account_id=self.account.id, comment_id=created_comment.id)
    ...
    def test_comment_isolation_between_accounts(self) -> None:
        ...
        # Test that each account can only see their own comments
        pagination_params = PaginationParams(page=1, size=10, offset=0)
        
        get_params1 = GetCommentsByTaskParams(
            account_id=self.account.id,
            task_id=self.task.id,
            pagination_params=pagination_params
        )
        account1_result = CommentService.get_comments_by_task(params=get_params1)

        get_params2 = GetCommentsByTaskParams(
            account_id=other_account.id,
            task_id=other_task.id,
            pagination_params=pagination_params
        )
        account2_result = CommentService.get_comments_by_task(params=get_params2)
        ...
        # Test that account1 cannot access account2's comment
        get_params = GetCommentParams(account_id=self.account.id, comment_id=account2_comment.id)
        with self.assertRaises(CommentNotFoundError):
            CommentService.get_comment(params=get_params)

    def test_comments_isolated_by_task(self) -> None:
        ...
        task1_result = CommentService.get_comments_by_task(params=get_params1)
        ...
        task2_result = CommentService.get_comments_by_task(params=get_params2)

        assert len(task1_result.items) == 1
        assert task1_result.items[0].id == task1_comment.id
        assert task1_result.items[0].content == "Comment for task 1"

        assert len(task2_result.items) == 1
        assert task2_result.items[0].id == task2_comment.id
        assert task2_result.items[0].content == "Comment for task 2"